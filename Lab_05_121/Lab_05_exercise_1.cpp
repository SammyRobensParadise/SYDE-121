...
int main() {
	int rows =0; //  rows that the diamond comsists of
	...
	do {
		...
		cin >> maxlength;
	} while( maxlength <= 0 || maxlength >= 20 || maxlength%2 ==0 );
	maxlength = maxlength/2;
	for( rows = -maxlength; rows <= maxlength; rows++) {
		for(columns = -maxlength; columns <= maxlength; columns++) {
			if (abs(rows)+ abs(columns) == maxlength) {
				cout<< "*";
			} else {
				cout << " ";
			}
