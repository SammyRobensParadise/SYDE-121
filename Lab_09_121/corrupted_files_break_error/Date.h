...
class Date {
	public:
		...
		//This function takes the day of a flight and calculates the time
		// that a person would have to by their ticket in advance to get their ticket.
		void backup();
		// Changes a valid date to the next date.
		// Exits if invalid date encountered.
		void check(bool a) const;
